...
DebugTask (void *pvParameters)
{
    ...
    while (1)
    {
        if(xQueueReceive(g_pDebugQueue, &(SERIAL_VALUE), 0) == pdPASS) // Receive from the debug queue and check it was successful.
        {
            ...
            xSemaphoreGive(xUARTSemaphore); // Return the mutex for the UART.
        }
